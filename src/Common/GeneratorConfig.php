...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('infyom_generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('infyom_generator.namespace.model', 'App\Models').$prefix;
        $this->nsDataTables = config('infyom_generator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'infyom_generator.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
            'infyom_generator.namespace.api_controller',
            'App\Http\Controllers\API'
        ).$prefix;
        $this->nsApiRequest = config('infyom_generator.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('infyom_generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('infyom_generator.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('infyom_generator.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('infyom_generator.namespace.controller', 'App\Http\Controllers').$prefix;
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'infyom_generator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('infyom_generator.path.model', app_path('Models/')).$prefix;

        $this->pathDataTables = config('infyom_generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'infyom_generator.path.api_controller',
            app_path('Http/Controllers/API/')
        ).$prefix;

        $this->pathApiRequest = config(
            'infyom_generator.path.api_request',
            app_path('Http/Requests/API/')
        ).$prefix;

        $this->pathApiRoutes = config('infyom_generator.path.api_routes', app_path('Http/api_routes.php'));

        $this->pathApiTests = config('infyom_generator.path.api_test', base_path('tests/'));

        $this->pathApiTestTraits = config('infyom_generator.path.test_trait', base_path('tests/traits/'));

        $this->pathController = config(
            'infyom_generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('infyom_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('infyom_generator.path.routes', app_path('Http/routes.php'));

        $this->pathViews = config(
            'infyom_generator.path.views',
            base_path('resources/views/')
        ).$viewPrefix.$this->mSnakePlural.'/';

        $this->modelJsPath = config(
                'infyom_generator.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('infyom_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('infyom_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        $this->options['softDelete'] = config('infyom_generator.options.softDelete', false);
    ...
    {
        $this->prefixes['route'] = explode('/', config('infyom_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('infyom_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('infyom_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('infyom_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('infyom_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('infyom_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('infyom_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('infyom_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('infyom_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
